    ...
    <div className="min-h-[85vh] flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-green-800 via-green-700 to-emerald-500 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 xl:px-20">
          ...
          <h2 className="text-4xl font-bold text-white mb-4">Portal Pacientes</h2>
          <p className="text-lg text-white/70 mb-8 max-w-md">
            Tu salud en un solo lugar. Accedé a tus turnos, resultados y consultas de forma simple y segura.
          </p>
          <div className="space-y-4">
            {[
              { icon: '📅', text: 'Reservá turnos con tu médico' },
              { icon: '📄', text: 'Accedé a tus estudios y recetas' },
              { icon: '💬', text: 'Chat y videoconsultas' },
              { icon: '👨‍👩‍👧', text: 'Gestión de grupo familiar' },
            ].map((item) => (
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12 bg-gray-50">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="mt-4 text-2xl font-bold text-gray-900">Hola, Paciente</h1>
            <p className="mt-1 text-sm text-gray-500">Accedé a tu historial y turnos médicos</p>
          </div>
          ...
          <div className="rounded-2xl bg-white p-8 shadow-lg ring-1 ring-gray-100">
            ...
            <div className="my-6 flex items-center gap-3"><div className="h-px flex-1 bg-gray-200" /><span className="text-xs text-gray-400">o con email</span><div className="h-px flex-1 bg-gray-200" /></div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="mt-1 relative">
                  ...
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                    className="block w-full rounded-xl border border-gray-200 pl-10 pr-4 py-3 text-sm outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20" placeholder="tu@email.com" />
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between">
                  <label className="block text-sm font-medium text-gray-700">Contraseña</label>
                  <button type="button" onClick={() => { setShowForgot(true); setForgotEmail(email); }} className="text-xs text-green-600 hover:underline">
                ...
                <div className="mt-1 relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)}
                    className="block w-full rounded-xl border border-gray-200 pl-10 pr-10 py-3 text-sm outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20" placeholder="••••••••" />
              ...
              <button type="submit" disabled={loading}
                className="w-full rounded-xl bg-green-600 py-3 text-sm font-semibold text-white transition hover:bg-green-700 disabled:opacity-50 shadow-sm">
                {loading ? (
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
              {forgotSent ? (
                <div className="text-center py-4">
                  ...
                  <button onClick={() => { setShowForgot(false); setForgotSent(false); }} className="mt-6 w-full rounded-xl bg-green-600 py-3 text-sm font-semibold text-white hover:bg-green-700 transition">
                    Entendido
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <form onSubmit={handleForgotPassword} className="mt-4 space-y-4">
                    <input type="email" required value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)}
                      className="block w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20" placeholder="tu@email.com" />
                    <div className="flex gap-3">
                      ...
                      <button type="submit" disabled={forgotLoading} className="flex-1 rounded-xl bg-green-600 py-3 text-sm font-semibold text-white hover:bg-green-700 transition disabled:opacity-50">
